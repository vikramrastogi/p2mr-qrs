...
| Estimate | p99 ms |
| --- | ---: |
| qrs_valid_saturated_block | 93.957 |
| qrs_worst_invalid_fixed_length_saturated_block | 107.736 |
| individual_schnorr_saturated_block | 434.150 |
| experimental_batch_schnorr_saturated_block | 304.741 |
...
| QRS worst p99 below individual Schnorr p99 | pass | qrs_worst=107.736 ms, individual_schnorr=434.150 ms |
| QRS worst p99 below experimental batch Schnorr p99 | pass | qrs_worst=107.736 ms, experimental_batch=304.741 ms |
| Experimental batch tightens individual Schnorr baseline | pass | experimental_batch=304.741 ms, individual_schnorr=434.150 ms |

## QRS Depth Sensitivity

| Field | Value |
| --- | ---: |
| qrs_depth0_p99_ms | 93.957 |
| qrs_depth128_p99_ms | 62.322 |
...
- Experimental batch speedup vs individual: 1.425x
- QRS valid exceeds a hypothetical batch baseline if speedup is greater than 4.621x.
- QRS worst-invalid exceeds a hypothetical batch baseline if speedup is greater than 4.030x.

| Hypothetical speedup vs individual | Hypothetical Schnorr p99 ms | QRS valid exceeds | QRS worst-invalid exceeds |
| ---: | ---: | --- | --- |
| 1.5x | 289.433 | false | false |
| 2.0x | 217.075 | false | false |
| 2.5x | 173.660 | false | false |
| 3.0x | 144.717 | false | false |
| 4.0x | 108.537 | false | false |
| 5.0x | 86.830 | true | true |
...
| --- | --- | ---: | ---: | --- |
| qrs_worst_exceeds_individual_schnorr | false | 434.150 | 107.736 | fallback trigger if QRS worst-invalid p99 exceeds individual Schnorr p99 |
| qrs_worst_exceeds_2_5x_hypothetical_reviewed_batch_schnorr | false | 173.660 | 107.736 | fallback trigger if QRS worst-invalid p99 exceeds a 2.5x hypothetical reviewed-batch-Schnorr baseline |
| qrs_worst_exceeds_3_0x_hypothetical_reviewed_batch_schnorr | false | 144.717 | 107.736 | fallback trigger if QRS worst-invalid p99 exceeds a 3.0x hypothetical reviewed-batch-Schnorr baseline |
| qrs_worst_exceeds_reviewed_public_batch_schnorr | unavailable | unavailable | 107.736 | fallback trigger if a reviewed public batch-Schnorr implementation becomes available and QRS exceeds it |
| bitcoin_core_validation_path_overhead | unavailable | unavailable | 107.736 | fallback trigger requires Bitcoin Core validation-path integration evidence; current harness models that path only |
